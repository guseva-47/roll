...
describe('AppController (e2e)', () => {
    ...
    const otherUser = {
        jwtToken: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVmYjE2NTAxYjMwNGU2NDY0NDFjZTJkYiIsImVtYWlsIjoibGV2aXNob2tAZ21haWwuY29tIiwiaWF0IjoxNjA1NDYxMjQ5LCJleHAiOjE2MDU2NzcyNDl9.VOmVi0kf5MkpvNPP4wEvwDVoiYIUecv1i_erWY5xqcE',
    ...
    describe('', () => {
        ...
        it('/ok (GET)', async (done) => {
            await request(app.getHttpServer())
                ...
                .expect(200)
                .then(response => {
                    if (response.body.email !== me.email) done.fail(`Unexpected email. ${response.body.email}`)
                })
        });

        // it('/:id (GET) profile', async (done) => {
        //     await getProfile(me.id, me.jwtToken).then(response => {
        //         if(response.body.email != me.email) done.fail('Unexpected email.');
        //     })
        // })
        
        // it('/:id (GET) another user\'s profile', async (done) => {
        //     await getProfile(otherUser.id, me.jwtToken).then(response => {
        //         if (response.body.email != otherUser.email) done.fail('Unexpected email.');
        //     })
        // })
        ...
        //         if (response.body.email != email) done.fail('Unexpected email.');
        //     })
        //     await editProfile(oldUserDto, me.id, me.jwtToken).then(response => {
        //         if (response.body.email != me.email) done.fail('Unexpected email.');
        //     });
        // })
        ...
        //     await getProfile(otherUser.id, me.jwtToken)
