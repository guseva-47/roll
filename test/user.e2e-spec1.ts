    ...
    describe('get profile', () => {
        it('/ok (GET)', async done => {
            await request(app.getHttpServer())
                ...
                .expect(200)
                .then(response => {
                    if (response.body.email !== me.email)
        ...
        it("/:id (GET) another user's profile", async done => {
            await getProfile(otherUser.id, me.jwtToken).then(response => {
                if (response.body.email !== otherUser.email)
                    done.fail(`Unexpected (${response.body.email}) email.`);
                done();
            });
        });
        ...
        it("/:id (GET) closed user's profile", async done => {
            ...
            await editProfile(userDto, otherUser.id, otherUser.jwtToken).then(
                response => {
            ...
            await editProfile(oldUserDto, otherUser.id, otherUser.jwtToken).then(
                response => {
