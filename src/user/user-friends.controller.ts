...
export class UserFriendsController {
    constructor(private friendsService: UserFriendsService) {}
    ...
    async getSubscribers(
        ...
        const idMe = req.user.id;
        if (idMe === idSomeUser) return await this.friendsService.getSubscribers(idMe);
    ...
    async getSubscriptions(
        ...
        const idMe = req.user.id;
        if (idMe === idSomeUser) return await this.friendsService.getSubscriptions(idMe);
    ...
    @Post('approvesub')
    @UseGuards(JwtAuthGuard)
    async approveSubscribe(
        @Body() someUser: { id: string },
        @Request() req,
    ): Promise<IUser> {
        const idMe = req.user.id;
