...
export class SheetService {
    ...
    private _getSheetFactory(gameSystem: gameSystemEnum): ISheetFactory {
        this.logger.log(
            `_getSheetFactory(): Создание фабрики нужного типа игры -${gameSystem}.`,
        );
    ...
    createNPCSheet(gameSystemType: gameSystemEnum): INPCSheet {
        this.logger.log(
            `createNPCSheet(): INPCSheet создание листа неигрока для игры с типом ${gameSystemType}.`,
        );
    ...
    createPlayerSheet(gameSystemType: gameSystemEnum): IPlayerSheet {
        this.logger.log(
            `createNPCSheet(): IPlayerSheet создание листа игрока для игры с типом ${gameSystemType}.`,
        );
        const gameSystem: ISheetFactory = this._getSheetFactory(gameSystemType);
        if (!gameSystem) throw new BadRequestException();
    ...
    createSheets(
        gameSystemType: gameSystemEnum,
        countNPCSheet: number,
        countPlayerSheet: number,
    ): Array<any> {
        this.logger.log(
            `createNPCSheet(). Создать ${countNPCSheet} листов для неигровых персонажей и ${countPlayerSheet} листов игровых персонажей`,
        );
        const gameSystem: ISheetFactory = this._getSheetFactory(gameSystemType);
        if (!gameSystem) throw new BadRequestException();
        const sheets = [];
        for (let i = 0; i < countNPCSheet; i++) sheets.push(gameSystem.createNPCSheet());
