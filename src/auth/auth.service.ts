...
export class AuthService {
    ...
    constructor(
        ...
        private readonly refreshTokenModel: Model<IJwtRefreshToken>,
    ) {}
    ...
    async login(userDto: CreateUserDto): Promise<ITokenObject> {
        ...
        if (!ishave)
            existedUser = await this.usersService
                .createUser(userDto)
                .catch(e => console.error(e));
        else
            existedUser = await this.usersService
                .findByCreateUserDto(userDto)
                .catch(e => console.error(e));
    ...
    private async saveNewRefreshTokenInDB(
        userId: string,
        refreshToken: string,
    ): Promise<IJwtRefreshToken> {
        const tokenRecord: JwtRefreshPayloadDto = this.createNewRefreshTokenPayload(
            userId,
            refreshToken,
        );
            ...
            const tokenFromDB = await this.refreshTokenModel
                ...
                .orFail(
                    new NotFoundException(
                        'The refresh token was not found by the user ID.',
                    ),
                );
    ...
    private createNewRefreshTokenPayload(
        userId: string,
        refreshToken: string,
    ...
    private getExpiredTime(start_date: Date): Date {
        ...
        const expiredAt = moment(start_date)
